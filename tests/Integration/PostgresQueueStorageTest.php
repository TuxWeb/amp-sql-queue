...
final class PostgresQueueStorageTest extends TestCase
{
    private PostgresConnectionPool $pool;

    protected function setUp(): void
    {
        $dsn = getenv('AMP_SQL_QUEUE_POSTGRES_DSN');
        if ($dsn === false || $dsn === '') {
            self::markTestSkipped('AMP_SQL_QUEUE_POSTGRES_DSN is not set.');
        }

        $this->pool = new PostgresConnectionPool(PostgresConfig::fromString($dsn));
        $this->pool->query('DROP TABLE IF EXISTS amp_queue_failed_jobs');
        $this->pool->query('DROP TABLE IF EXISTS amp_queue_jobs');
        $this->pool->query((string) file_get_contents(__DIR__ . '/../../migrations/postgres/001_create_queue_tables.sql'));
    }

    public function testDispatchReserveAndAck(): void
    {
        $queue = new Queue(new PostgresQueueStorage($this->pool));
    ...
    public function testConcurrentReservationsDoNotReturnSameJob(): void
    {
        $queue = new Queue(new PostgresQueueStorage($this->pool));
        ...
        self::assertNotSame((string) $jobs[0]->id, (string) $jobs[1]->id);
    }
    ...
    public function testFailRequeuesJobWithDelayUntilMaxAttempts(): void
    {
        ...
        $row = $this->pool->execute(
            "SELECT status, attempts, last_error->>'message' AS message, available_at > NOW() AS delayed FROM amp_queue_jobs WHERE id = ?",
            [$job->id],
        )->fetchRow();
        ...
        self::assertTrue((bool) ($row['delayed'] ?? false));
    }
    ...
    public function testFailMovesJobToDeadLetterWhenAttemptsAreExhausted(): void
    {
        ...
        $failedRow = $this->pool->execute(
            "SELECT original_job_id, attempts, last_error->>'message' AS message FROM amp_queue_failed_jobs WHERE original_job_id = ?",
            [$job->id],
        )->fetchRow();
    ...
    public function testExpiredLeaseIsRecoveredForReservation(): void
    {
        ...
        $this->pool->execute(
            "UPDATE amp_queue_jobs SET locked_until = NOW() - INTERVAL '1 second' WHERE id = ?",
            [$job->id],
        );
    ...
    public function testExtendLeaseKeepsProcessingJobLockedLonger(): void
    {
        ...
        $row = $this->pool->execute(
            "SELECT status, locked_by, locked_until > NOW() AS locked FROM amp_queue_jobs WHERE id = ?",
            [$job->id],
        )->fetchRow();

        self::assertSame('processing', $row['status'] ?? null);
        self::assertSame('worker-1', $row['locked_by'] ?? null);
        self::assertTrue((bool) ($row['locked'] ?? false));
    }
